...
test('1C accounting catalog has a single 30-question test with protected answer keys', () => {
  const track = catalog.find(item => item.id === '1c-accounting');
  assert.equal(track.tasks.length, 1);
  const task = track.tasks[0];
  assert.equal(task.id, 'test');
  assert.equal(task.questions.length, 30);
  assert.equal(task.questions.filter(question => question.q.includes('встроенного языка 1С')).length, 2);
