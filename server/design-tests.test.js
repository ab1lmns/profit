...
for (const id of ['industrial-design']) {
  test(`${id} has one 30-question test with private answers`, () => {
    ...
    assert.deepEqual(score({ trackId: id, tasks: track.tasks }, { answers }), { correct: 3, total: 30, practicePending: false });
  });
}
